...
import { DrawSettings, PrizeAwardable, User, UserDrawResult } from '../types';
import { readAddressesFromFile } from './utils/createRandomUsers';
...
async function runDrawCalculatorEqualBalances(
    numberOfPrizePeriods: number,
    numberOfUsers: number,
    balancePerUser: BigNumber,
    drawSettings: DrawSettings,
): Promise<Map<string, UserDrawResult[]>[]> {
    ...
    const _picks = [...new Array<number>(numberOfPicksPerUser.toNumber()).keys()].map((num) =>
        BigNumber.from(num),
    );
    ...
    for (
        let currentPrizePeriod = 0;
        currentPrizePeriod < numberOfPrizePeriods;
        currentPrizePeriod++
    ) {
        let draw = {
            drawId: BigNumber.from(currentPrizePeriod),
            winningRandomNumber: BigNumber.from(
                ethers.utils.solidityKeccak256(['address'], [ethers.Wallet.createRandom().address]),
            ),
        };
    ...
    resultsMaps.forEach((resultMap: any) => {
        ...
        userResults = userResults.mergeWith(
            (oldVal: UserDrawResult[], newVal: UserDrawResult[], key: string): any => {
...
async function run() {
    ...
    const drawSettings: DrawSettings = {
        ...
        bitRangeSize,
        distributions: [
            333333333, // 25,000 / 75,000  => $25000 prize
            2133333, // (1,000 / 75,000) * 16 ((2 ^ bitRangeSize) ^ distributionIndex => $1000 prize
            170666, // (5 / 75,000) * 256 winners => $5 prize
        ],
        pickCost,
    ...
    const results: Map<string, UserDrawResult[]>[] = await runDrawCalculatorEqualBalances(
        1,
        1000,
...
function parseAndSaveCSVResults(
    results: Map<string, UserDrawResult[]>,
    drawSettings: DrawSettings,
    fileName: string,
) {
    ...
    results.forEach((winners, index) => {
        ...
        let prizeDistributions: Array<number> = new Array(drawSettings.distributions.length).fill(
            0,
        );

        winners.forEach((winner) => {
            totalPrizeAwardableForPeriod = totalPrizeAwardableForPeriod.add(
                winner.drawResult.totalValue,
            );
        ...
        csvFormatString = csvFormatString.concat(
            `,${utils.formatEther(totalPrizeAwardableForPeriod)}`,
        );
