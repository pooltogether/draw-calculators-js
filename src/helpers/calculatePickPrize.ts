...
export function calculatePickPrize(randomNumberThisPick: string, winningRandomNumber: BigNumber, prizeDistribution: PrizeDistribution, draw: Draw): PickPrize | undefined {
    ...
    for(let matchIndex = 0; matchIndex < prizeDistribution.matchCardinality; matchIndex++){ 
        ...
        if(!findBitMatchesAtIndex(bigRando, winningRandomNumber, matchIndex, prizeDistribution.bitRangeSize)){
